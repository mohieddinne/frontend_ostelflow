...
class FuseUtils {
  ...
  static hasPermission(authArr, userRole) {
    if (authArr === null || authArr === undefined) {
      return true;
    } else if (authArr === "onlyGuest") {
      return userRole === "";
    }
    // Init the objection
    let auth = { slug: "", permission: "" };
    if (typeof authArr === "string") {
      auth.slug = authArr;
      auth.permission = ["can_view", "can_view_own"];
    } else if (typeof authArr == "object") {
      auth.slug = authArr.slug;
      auth.permission = authArr.permission;
    }
    ...
    if (userRole[auth.slug] === null || userRole[auth.slug] === undefined) {
      return false;
    }
    const accessSlug = userRole[auth.slug];
    if (Array.isArray(auth.permission)) {
      return !!auth.permission.find((p) => accessSlug[p]);
    }

    if (!accessSlug[auth.permission]) {
      return false;
    }

    return userRole[auth.slug][auth.permission];
  }
}

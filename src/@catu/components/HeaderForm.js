...
const Actions = (props) => {
  ...
  // Check submittable
  const { reduxStore, handleSubmit, toggleEditable, single_button } = props;
    ...
    <>
      {single_button ? (
        <Button
          ...
          color="secondary"
          disabled={
            !canSubmit || isSaved || loading || props.submitAction === "disable"
          }
...
const Title = ({ reduxStore, defaultTitle }) => {
  ...
  return (
    <Typography className="text-16 sm:text-20 truncate">
      {loading && !name ? t("loading") : name || defaultTitle || ""}
    </Typography>
  );
};
      ...
      {submitAction && (
        <FuseAnimate animation="transition.slideRightIn" delay={300}>
          <Actions
            ...
            single_button={options.single_button}
            reduxStore={reduxStore}
